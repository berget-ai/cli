 ...
 */
async function mergeConfigurations(currentConfig: any, latestConfig: any): Promise<any> {
  try {
...
Return ONLY the merged JSON configuration, no explanations.`
    ...
    try {
      ...
      console.warn(chalk.yellow('⚠️  AI response invalid, using fallback merge'))
      return fallbackMerge(currentConfig, latestConfig)
    }
  } catch (error) {
    ...
    .action(async (options) => {
      try {
        ...
        if (!options.yes) {
          ...
          console.log(chalk.dim(`Environment: ${path.join(process.cwd(), '.env')} will be updated`))
  ...
  code
    ...
    .action(async (options) => {
      try {
        ...
        console.log(chalk.dim(`  Agents: ${Object.keys(currentConfig.agent || {}).length} configured`))
        ...
        if (needsUpdate) {
          ...
          if (!currentConfig.provider?.berget?.models?.['deepseek-r1']?.maxTokens) {
        ...
        console.log(chalk.cyan('1) Replace - Use latest configuration (your customizations will be lost)'))
        console.log(chalk.cyan('2) Merge  - Combine latest updates with your customizations (recommended)'))
        ...
        if (await confirm(`\nProceed with ${mergeChoice}? (Y/n): `, options.yes)) {
          try {
            ...
            if (!hasGit()) {
              ...
              console.log(chalk.green(`✓ Backed up current config to ${path.basename(backupPath)}`))
            }
