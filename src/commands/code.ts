    ...
    .action(async (prompt: string, options: any) => {
      try {
        ...
        if (isLocalMode) {
          ...
          console.log(chalk.dim('Using local API: http://localhost:3000'))
        } else if (isStageMode) {
          env.BERGET_API_URL = 'https://api.stage.berget.ai'
          console.log(chalk.dim('Using stage API: https://api.stage.berget.ai'))
        } else {
          env.BERGET_API_URL = 'https://api.berget.ai'
        }
