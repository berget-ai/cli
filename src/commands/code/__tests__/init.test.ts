...
describe('runInit', () => {
  describe('happy path', () => {
    it('sets up opencode project without existing config', async () => {
      ...
      expect(config.plugin).toContain('@bergetai/opencode-auth@^1.0.26');
    });
  ...
  "plugin": ["other-plugin"]
    ...
    it('shows no changes needed when config is already up to date', async () => {
        ...
        JSON.stringify(
          {
            ...
            plugin: ['@bergetai/opencode-auth@^1.0.26'],
          },
          null,
          2,
        ) + '\n',
      ...
      expect(config.plugin).toEqual(['@bergetai/opencode-auth@^1.0.26']);
